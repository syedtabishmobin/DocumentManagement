...
| `STORY-P1-006` | `FIX READY — QA PENDING` | Issue #58 / PR #59 now count post-lease worker loss in the same three-attempt execution-generation budget. The third lost lease becomes the single terminal/dead-letter outcome; above-limit delivery creates no fourth run or effect; a newer replay generation repairs without reactivating prior attempts. File-backed below/at/above-limit and restart tests pass, and equivalent real-PostgreSQL restart coverage is committed for protected CI. | Keep PR #59 unmerged until protected CI and independent exact-candidate QA pass defect #60, both story ACs, real-PostgreSQL evidence, and P1-001–005 regression. |
