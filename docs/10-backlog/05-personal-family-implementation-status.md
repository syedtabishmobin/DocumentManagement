...
| `STORY-P1-005` | `IN IMPLEMENTATION` | Issue #55 extends the durable ingestion case and existing suspected-clinical fence; it does not treat the legacy heuristic as sufficient. | Add deterministic synthetic safety assessment, scanner-unavailable/malicious containment, truthful case/event/audit state, generic UX, restart/replay and independent adversarial QA without activating a production scanner or clinical disposition. |
