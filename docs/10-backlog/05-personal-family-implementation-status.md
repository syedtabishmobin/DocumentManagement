...
| `STORY-P1-003` | `IN IMPLEMENTATION` | Issue #46 extends the existing deny-by-default explicit-grant evaluator rather than replacing it. Current durable reload and cross-workspace/no-grant tests are reusable evidence. | Field/edge/action scope, current-output/effect reauthorization, epoch/revocation propagation, privacy-safe decision evidence, applicable canonical conformance, independent security/functional QA and protected real-PostgreSQL regression. |
