...
| `STORY-P1-006` | `DEV COMPLETE — QA REQUIRED` | Issue #58 adds provider-neutral durable stage execution keys, bounded leases and attempt history, immutable message fingerprints, duplicate/stale/out-of-order reconciliation, crash-window convergence, bounded retry/dead-letter repair, current authorization/containment/deletion/cancellation/route/cost fences, transactional `EVT-P1-006` evidence, and canonical API-P1-142/143 Job query/cancellation views. Developer verification passes file restart, HTTP, privacy-safe event, repair and P1-001–005 regression coverage. | Independent Functional/Security/Resilience QA, exact-candidate real-PostgreSQL protected CI, defect/retest loop, merge, Stage/BA/UAT and parent feature/outcome completion remain required. |
