...
| `STORY-P1-004` | `IN IMPLEMENTATION` | Issue #50 extends the existing file, camera-compatible and manual capture preview with canonical durable ingestion cases rather than replacing the capture surfaces. | Complete route convergence, resumable/receipt behavior, content validation, accessibility/mobile evidence, independent QA and protected PostgreSQL regression; production artifact activation remains separately gated. |
