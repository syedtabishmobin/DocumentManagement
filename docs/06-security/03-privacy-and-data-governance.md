...
| `PRIV-P1-025` | Recovery MUST NOT reveal or transfer private resources, consent, keys, ownership, or grants under approved Phase 1 `DEC-038`; future assurance/challenge rules require a later governed decision. | `REQ-P1-TRUST-008` | Recovery takeover suite. |
...
6. `DEC-032` and `DEC-036` remain visibly blocked; approved `DEC-038` keeps its Phase 1 recovery/ownership-transfer capability visibly unavailable; `DEC-049`–`053` have updated contracts and require their specified placement, encryption, deletion, legal, and release evidence.
