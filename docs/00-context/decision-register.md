...
| `DEC-046` | PROPOSED | Do not transmit temporary passwords by email or SMS. Send a short-lived, single-use, audience-bound invitation link and optional one-time verification code; require the invitee to establish their own password or passkey on first redemption. This preserves the requested first-login credential setup without creating reusable credentials in messages. | Security recommendation awaiting product-owner confirmation, 28 August 2026 |
