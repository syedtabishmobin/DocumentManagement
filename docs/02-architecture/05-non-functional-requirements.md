...
| `NFR-P1-032` | Account/workspace recovery safety under the approved Phase 1 unavailability decision: unapproved recovery/owner-transfer routes denied / all attempted routes. | **100% denied**, zero support/email/family-relationship bypass, and no production recovery/owner-transfer capability enabled unless a later governed decision supersedes `DEC-038` and this NFR is revised with success-time/assurance targets. | Route inventory, abuse tests, support tooling review, recovery attempt audit. | APPROVED CAPABILITY FENCE under `DEC-038`; ZERO TOLERANCE for bypass. |
...
- the approved `DEC-038` Phase 1 recovery/ownership-transfer unavailability fence remains enforced; all `DEC-049`–`054` placement, encryption, deletion, client, and release prerequisites are evidenced;
