...
| `OPS-CICD-P1-014` | Every candidate MUST run the repository gates from the repository root through `pnpm verify`, including `pnpm verify:framework`, specification/API/reference-data/test-traceability validators, TypeScript checks, tests, and builds. Flutter and infrastructure gates run in CI as defined by `.github/workflows/ci.yml`. |
...
| `OPS-CICD-P1-026` | A release MUST NOT activate a route or behavior fenced by `DEC-031`–`055`; configuration defaults, environment variables, migrations, or stale feature state cannot bypass an approved activation or release gate. |
...
Candidate evidence retention follows an approved versioned policy and remains separate from the `DEC-053` document Trash/purge lifecycle. Evidence remains classified and residency-controlled under `DEC-049`. No build, gate, waiver, configuration, or promotion may activate account/workspace recovery or ownership transfer prohibited by `DEC-038`. Build logs and reports are not a shadow source/content store. A finding ticket references protected evidence where needed and never copies raw household or secret material into ordinary CI output.
