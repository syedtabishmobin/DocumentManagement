...
| Version | `0.1` |
| Status | **DRAFT — all RPO/RTO/cadence targets are provisional; product-owner, architecture, security, privacy, operations, and data approval required** |
| Product phase | Phase 1 — Personal and Family |
| Updated | 26 August 2026 |
| Primary trace | `ARCH-P1-026`–`032`, `039`–`045`, `DOM-P1-048`–`052`, `SEC-P1-009`–`012`, `027`–`030`, `NFR-P1-026`–`032`, `DEC-038`–`040` |

## 1. Purpose, authority, and provisional status

This document defines provider-neutral backup classes, recovery objectives, protected backup manifests, restore/failover gates, and exercise evidence. It selects no backup, replication, storage, key, database, cloud, region, failover, or disaster-recovery product/topology.

All numeric objectives below are copied from [`ARCH-NFR-001`](../02-architecture/05-non-functional-requirements.md) and remain **PROVISIONAL**. They do not approve a customer promise or a physical solution. `DEC-038` blocks user/account/workspace recovery design, `DEC-039` blocks backup-expiry/deletion-duration promises, and `DEC-040` blocks presumed backup/DR/failover regions and processors.
...
| Secret/key references and protected recovery material | Separated security authority | Dedicated encrypted recovery contract and custody; never embedded with data backup | Approved custodians/ceremony, version/status, route, disable/rotation, no `DEC-038` bypass |
...
| `OPS-DR-P1-002` | Backup and recovery mechanisms MUST be provider-neutral capabilities with versioned manifests, evidence, failure/deletion behavior, and portability tests; no product or region is selected by this standard. |
...
| `OPS-DR-P1-012` | Retention, backup frequency beyond an approved NFR, backup expiry, residual, and audit-minimization rules MUST be versioned policy. No duration or full-deletion completion claim is inferred while `DEC-039` remains open. |
...
| `OPS-DR-P1-014` | Backup copies containing deletion-fenced lineage remain unavailable to ordinary access and restore. Backup expiry/purge acknowledgement is per role/generation and cannot by itself mark the platform `DeletionCase` complete. |
...
| `OPS-DR-P1-018` | A failover or alternate recovery location/processor is a new route decision. It MUST be declared eligible for every data/processing/support/telemetry/backup class; availability cannot create a cross-border exception under `DEC-040`. |
...
| `OPS-DR-P1-032` | No backup, restore, replica, snapshot, key, failover, or DR mechanism may close `DEC-038`, invent `DEC-039` durations/completion, or presume a `DEC-040` route. Unknown state keeps the affected recovery path disabled. |
