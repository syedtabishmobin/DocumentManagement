...
Registration is not activation. A document provider is active only after its adapter implements authorization-code exchange, state and PKCE validation, encrypted token custody, minimal-scope consent, disconnect/revocation, deletion fencing, audit, error recovery, and the tests required by [`API-CON-001`](../05-api/04-connector-contracts.md). The current deployment deliberately keeps `DM_EXTERNAL_CONNECTORS=disabled` and `DM_CONNECTOR_ADAPTERS_READY=false`. The separately governed Product Authority/UAT email channel is enabled only because Issue #18 passed its own narrower conformance gate.
