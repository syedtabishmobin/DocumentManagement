...
Material agent-generated control-plane records MUST expose three consistent levels in this exact order: a compact role-oriented display identity at the beginning, collapsible execution details immediately after that identity and before the record body, and final hidden versioned metadata for machine joins. Human-facing display IDs MUST be stable, concise, and separate from raw runtime/session identifiers. Hidden metadata MUST preserve both identities when available, plus run, parent, work item, capability, skill, tool, revision, and environment correlation. A validator MUST reject missing levels, disagreement between levels, unregistered or unauthorized claims, duplicate IDs, misplaced or out-of-order levels, and display identities that are not assigned to the run. Project-specific identity prefixes, assignments, role labels, and tracker rendering belong in project configuration.
