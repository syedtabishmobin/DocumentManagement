...
The provider-console setup, exact permissions, Key Vault secret references, verified development state, remaining corrections, and activation checklist are maintained in [`OPS-PROVIDER-001`](../docs/09-devops/08-external-provider-setup.md). As verified on 29 August 2026, Microsoft registration, ACS Email foundations, the Google development test user, and Box read-only scope are prepared. Google still needs distinct legal URLs/logo plus saved Data Access scopes; Dropbox needs implicit/OIDC scope correction and secret rotation after review. Registration metadata must not be interpreted as a working adapter.
