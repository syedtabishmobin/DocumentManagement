...
Register the display/runtime run pairing in `.agents/state/agent-display-assignments.json`, then use `python3 scripts/github_attribution.py wrap --body-file <body.md> ...` to generate the complete three-level material GitHub record. It begins with the visible role/display agent and display run, places collapsible parent/work/capability/skill/tool/revision/environment details immediately next and before the full body, and ends with hidden v2 metadata containing separate display and runtime identities. Run `python3 scripts/github_attribution.py validate <wrapped.md>` before publication; legacy v1-only or out-of-order records do not satisfy the material-record gate.
