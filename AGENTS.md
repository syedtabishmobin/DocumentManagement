...
For governed material work, record privacy-safe lifecycle events and use `pnpm agent:status`, `pnpm agent:tree`, or `pnpm agent:summary` for evidence-backed operations state. Do not put prompts, credentials, document/customer content, or arbitrary tool payloads into telemetry.
