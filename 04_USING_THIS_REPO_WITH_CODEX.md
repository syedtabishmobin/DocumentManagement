...
Keep the assignment `workItem` stable for the run, normally the governing Issue (`issue-32`). Use the human-facing `workItemLabel` to name the current artifact (`Issue #32` or `Issue #32 / PR #34`). Do not mutate the stable machine key when a PR is created; completed assignments remain valid historical correlation records.
