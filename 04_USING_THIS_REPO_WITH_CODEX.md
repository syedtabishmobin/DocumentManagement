...
python3 scripts/github_attribution.py render --agent-id <id> --run-id <id> --role-id <role> --work-item issue-<n> --capability-ids <ids> --skill-ids <ids> --tool-ids <ids>
```

Every material Codex-authored GitHub Issue, comment, pull-request description/review, QA result and test-evidence record must end with the normalized footer generated by `scripts/github_attribution.py`. The format is configured in `.agents/project/github-attribution.json`; do not invent a conversational substitute or include owner contact/Azure tenant values in a public record.
