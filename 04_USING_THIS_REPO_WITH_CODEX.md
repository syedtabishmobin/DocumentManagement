...
2. immediately after those lines and before the full record body, a collapsible execution block shows parent, work item, capabilities, skills, tools, commit and environment; and
3. the final hidden `doculyra-agent-meta:v2` block preserves separate display/runtime agent and run IDs for telemetry joins.

The validator joins the record to the assignment, team, capability, skill and tool registries and rejects inconsistent levels. The implementation agent must not edit a QA agent's conclusions or claim its visible identity; QA must publish or update its own independent records. Do not include owner contact/Azure tenant values in a public record.
