...
function connectorStatusLabel(status: ConnectorDescriptor["status"]) {
  if (status === "READY_TO_CONNECT") return "Ready for consent";
  if (status === "CONFIGURED_DISABLED") return "Configured · validation pending";
  return "Configuration required";
}

function connectorStatusDetail(status: ConnectorDescriptor["status"]) {
  if (status === "READY_TO_CONNECT") return "Provider configuration and adapter validation are complete";
  if (status === "CONFIGURED_DISABLED") return "Provider registration is prepared; connection remains disabled until security and lifecycle tests pass";
  return "Provider registration or required configuration is incomplete";
}
