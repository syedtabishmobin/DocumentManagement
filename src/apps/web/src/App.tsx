...
function Documents({ data, onUpload, onDelete }: { data: DashboardSnapshot; onUpload: () => void; onDelete: (id: string) => Promise<void> }) {
  ...
  return <><PageHead eyebrow="Secure, organised library" title="Documents" copy="Browse immutable originals by category, person and document. Open a dossier to preview the exact local version, extracted proposals and typed connections." action={<button className="primary" onClick={onUpload}><Upload size={18} /> Add document</button>} />
    ...
    {docs.length ? <div className="document-library"><aside className="category-tree"><span className="eyebrow">Library hierarchy</span><button className={category === "ALL" ? "active" : ""} onClick={() => setCategory("ALL")}><FolderSearch2 /><span><strong>All documents</strong><small>{active.length} visible</small></span></button>{categories.map((item) => <button className={category === item ? "active" : ""} key={item} onClick={() => setCategory(item)}><FolderSearch2 /><span><strong>{item}</strong><small>{active.filter((document) => document.category === item).length} document{active.filter((document) => document.category === item).length === 1 ? "" : "s"}</small></span></button>)}</aside><div className="category-groups">{groups.map((group) => <section key={group.category}><header><div><span className="eyebrow">Category</span><h2>{group.category}</h2></div><span>{group.documents.length} visible</span></header><div className="document-grid">{group.documents.map((document) => <DocumentCard key={document.id} document={document} data={data} onOpen={() => setSelectedDocument(document.id)} onDelete={onDelete} />)}</div></section>)}</div></div> : <Empty icon={FileText} title="No matching documents" copy="Add a local document or change your search and category filters." action={<button className="secondary" onClick={onUpload}><Upload size={17} /> Add document</button>} />}
