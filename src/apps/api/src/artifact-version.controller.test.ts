...
describe("immutable document version and artifact redemption HTTP boundary", () => {
  ...
  it("implements API-P1-122, 123, 125, 126, 127 and 128 without permanent byte URLs", async () => {
    ...
    try {
      ...
      const collection = await fetch(documentsUrl, { headers: headers("artifact-documents") }); expect(collection.status).toBe(200);
      ...
      const documentResponse = await fetch(`${documentsUrl}/${first.id}`, { headers: headers("artifact-document") }); expect(documentResponse.status).toBe(200); expect(await documentResponse.json()).toMatchObject({ document_id: first.id, current_version_id: currentVersionId });
      const versionsResponse = await fetch(`${documentsUrl}/${first.id}/versions`, { headers: headers("artifact-versions") }); expect(versionsResponse.status).toBe(200);
      ...
      const exactResponse = await fetch(`${documentsUrl}/${first.id}/versions/${currentVersionId}`, { headers: headers("artifact-version") }); expect(exactResponse.status).toBe(200); expect(await exactResponse.json()).toMatchObject({ document_id: first.id, document_version_id: currentVersionId, artifact_id: versions.items[0]!.artifact_id });
      ...
      expect(grantResponse.status).toBe(201); const grant = await grantResponse.json() as { artifact_access_grant_id: string; expires_at: string; resource_version_ref: string }; expect(grant.resource_version_ref).toBe(currentVersionId);
      ...
      expect(redemptionResponse.status).toBe(201); expect(redemptionResponse.headers.get("cache-control")).toBe("private, no-store");
